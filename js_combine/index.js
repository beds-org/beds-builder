...
var filterIgnore = function( ignore, id, origId ){
    ...
    mergePath = function( options, deps, base ){
        ...
        return deps.map(function( item, i ){
            ...
            //处理绝对路径
            if (origId.indexOf(".") != 0 && options.srcRootPath) {
                base = options.srcRootPath;
            } else {
                base = oBase;
            }
            return {
    ...
    modPathResolve = function( options, filePath ){
        ...
        var id = filePath, //filePath.match( rModId )[1]
    ...
    readDeps = function( options, parentDeps ){
        ...
        promiseArr = parentDeps.map(function( item ){
            return new Promise(function( resolve, reject ){
                ...
                // 根据模块后缀来匹配是否使用插件
                if(extName && !~extName.indexOf('.js') ){ //暂时注释掉该功能
                    resolve();
    ...
    pullDepsForDm = function( options, reg, contents ){
        var deps = [],
            matches, origId;
        reg.lastIndex = 0;
        while( (matches = reg.exec(contents)) !== null ){ //reg必须有g参数才能遍历索引
            var depArr = matches[1].match(/["'](.*?)['"]/g);
            depArr && depArr.forEach(function(e) {
                origId = e.replace(/['"]/g, '');
                if( origId && origId.slice(0, 4) !== 'http' ){
                    depPathResult = modPathResolve( options, origId );
                    deps.push({
    ...
    parseDeps = function( options, contents, modData ){
        ...
        if( !isSeajsUse ){
            if (options.simpleMode) {
                ...
                deps = pullDepsForDm( options, rRequireForDoumi, contents );
            }
        }
        // 解析seajs.use
        else{
    ...
    transform = function( options, modData, idMap ){
        ...
        var base = path.resolve( modData.path, '..' );
        // 标准模块
        if( !isSeajsUse ){
            contents = contents.replace( rRequire, function( $, _, $2 ){
                ...
                if( $2 && $2.slice(0, 4) !== 'http' ){
                    ...
                    if(depId.charAt(0) == '.') {
                        depId = path.resolve(base, depId);
                        depId = path.relative(options.srcRootPath, depId).replace(/[\/\\]+/g, '/') + ".js";
                    }
                    deps.push( depId );
            ...
            if (options.simpleMode && !filterIgnore( options.ignore, modData.id, origId )) {
                var id = modData.path.replace(/^.*?src[\/\\]+/, '').replace(/[\/\\]+/g, '/')
                if (deps.length) {
                    contents = "define('" + id + "',['" + deps.join("','") + "'], function(require, exports, module) {"  + contents + " })"
                } else {
                    contents = "define('" + id + "', function(require, exports, module) {"  + contents + " })"
    ...
    createStream = function( options ){
        ...
        if( options ){
            ...
            if (options.srcRootPath) {
                o.srcRootPath = options.srcRootPath;
            }
            if( options.ignore ){
